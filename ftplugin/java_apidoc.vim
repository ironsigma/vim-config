...
"-----------------------------------------------------------------------------
let javadoc_path="/Developer/ADC Reference Library/documentation/Java/Reference/1.5.0/doc/api,/Users/juanfrias/Development/Java/doc/Ostermiller"
" Windows users....
...
"-----------------------------------------------------------------------------
nmap <leader>jh :call OpenJavadoc()<CR><CR>
"-----------------------------------------------------------------------------
"
...
function! s:java_apidoc_search(allclasses, path, classname)
	let curr_buf = bufnr('%')
	let html_buf = 0
        let esc_allclasses = escape(a:allclasses, ' ')
        let esc_path = escape(a:path, ' ')
	execute(':view +0 '.esc_allclasses)
	call s:java_apidoc_searchHtml(esc_path, a:classname)
	let html_buf = bufnr('%')
	execute(':b '.curr_buf)
	execute(':bd '.html_buf)
endfunction
